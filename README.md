...
Все проверки доступа покрыты автоматическими тестами.
...
```
* Команда соберет образ, установит зависимости, выполнит миграции и загрузит фикстуры.
#### 3. Запуск тестов
```bash
docker compose run --rm app_test php bin/phpunit
```
* Все тесты (Unit, Integration, Functional) будут прогнаны внутри отдельного тестового контейнера. Тестовая база данных создается и настраивается автоматически.

#### 4. Доступ к приложению
* Приложение будет доступно по адресу: http://localhost:8080
* Для работы с основным приложением (dev) используйте:
```bash
